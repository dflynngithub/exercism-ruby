class Game
    ...
    def roll(pins)
        ...
        # Start a new frame if the previous one is full
        if @frames.last.frame_complete?
            Frame.new
        else

        # Push this pin record to a frame
        @frames.last.push pins
        p @frames
    ...
    class Frame
        ...
        # Define whether a frame is complete
        def frame_complete?
            @frame[1] != nil?() && @frame[2] != nil?()
        end
